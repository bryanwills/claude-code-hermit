...
- All WS mutations are gated by `ha_safety_mode` via `gateStructuralMutation` in `policy.ts`; reads never are. Under `strict` a mutation is refused (`blocked: true`) and surfaced as a proposal; under `ask` Claude Code requests native approval before execution. Every mutation writes an `audit-ha-ws-*` report to `.claude-code-hermit/raw/`.
