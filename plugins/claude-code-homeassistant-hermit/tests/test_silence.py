...
from ha_agent_lab.silence import compute_silence_summary
from ha_agent_lab.time_utils import days_since, parse_iso
...
def test_dead_automation_when_enabled_and_last_triggered_null(tmp_path):
    ...
    old = (NOW - timedelta(days=45)).isoformat()
    idx = _entity("automation.lights", state="on", last_changed=old, attrs={"last_triggered": None})
    result = compute_silence_summary(_normalized(idx), tmp_path, now=NOW)
...
def test_recently_enabled_automation_never_fired_not_dead(tmp_path):
    ...
    idx = _entity("automation.new", state="on", last_changed=recent, attrs={"last_triggered": None})
    result = compute_silence_summary(_normalized(idx), tmp_path, now=NOW)
