...
Skills self-advertise through their own `SKILL.md` descriptions — they are not catalogued here. The curated `php forge.php` commands cover the hot paths; run `php ${CLAUDE_PLUGIN_ROOT}/php/forge.php --help` for the full catalog. Any other SDK read goes through read-only generic dispatch, which accepts only methods on a closed allowlist and cannot mutate anything. Args are a JSON array on stdin, with IDs as bare numbers (SDK params are typed ints; `strict_types` rejects `"123"`):
