...
// Run with: bun test tests/hatch-skill.test.ts
//
...
if (fs.existsSync(SKILL)) {
  ...
  ok('runs preflight through core, keyed to its own plugin id',
    text.includes('domain-hatch preflight laravel-forge-hermit'));
  ...
  ok('records the operator\'s choice via ensure-target',
    text.includes('domain-hatch ensure-target laravel-forge-hermit --target'));
  ...
  ok('writes the block via sync-block',
    text.includes('domain-hatch sync-block laravel-forge-hermit'));
  ...
  // Scoped to the CORE floor — this hatch legitimately requires PHP >= 8.5.0.
  ...
  ok('still states its own PHP floor', /PHP 8\.5\+? is required/.test(text));

  ok('stamps its own version into _hermit_versions',
    text.includes('_hermit_versions["laravel-forge-hermit"]'));
  ok('the stamped value comes from the preflight verdict, not a literal',
    /_hermit_versions\["laravel-forge-hermit"\][\s\S]{0,60}self_version/.test(text));

  ok('names the block marker it hands to sync-block',
    text.includes('<!-- laravel-forge-hermit: Forge Workflow -->'));
}
...
if (fs.existsSync(TEMPLATE)) {
  ...
  ok('opening marker present', tpl.includes('<!-- laravel-forge-hermit: Forge Workflow -->'));
  ok('closing marker present', tpl.includes('<!-- /laravel-forge-hermit: Forge Workflow -->'));
}
