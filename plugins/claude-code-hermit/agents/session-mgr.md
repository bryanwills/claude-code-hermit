...
8. **Idle-task reconciliation + transition clear (single atomic runtime.json write).** If `idle_task` exists in runtime.json (set by heartbeat when an idle task was picked from IDLE-TASKS.md):
   ...
   In the same write, set all of: `idle_task: null`, `transition: null`, `transition_target: null`, `transition_started_at: null`, `session_state: "idle"`. Pre-compute next `session_id` (for the next task).
