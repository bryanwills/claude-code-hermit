...
- Everyday settings now need your own chat rather than any chat that can reach the hermit — the same chat trusted for pause, resume and status. On a shared home chat with no `allowed_users`, a bystander could previously change the model or switch the watchdog off while being refused a status read. `hermit-doctor` warns when a shared home chat carries this authority without an allowlist.
...
5. If this hermit's briefing chat is a group or server channel and the channel has no `allowed_users`, tell the operator once that every member of that chat can now change security-tier settings, and that setting `allowed_users` from a terminal restricts it to named operators. The next `/claude-code-hermit:hermit-doctor` reports the same thing as a `config` warning. Skip silently when the briefing chat is a 1:1 DM, an allowlist is already set, or a `maintainer_channel_id` is configured.
