...
### Added
- **Deterministic channel voice** — new `lib/channel-send.ts` (+ `channel-send.ts` CLI) is the first script-owned channel send: it resolves the eligible outbound channel, reads the bot token straight from that channel's `state_dir/.env`, and POSTs directly to Telegram/Discord — no model turn required. The watchdog now pushes an operator-language message on a restart, the first tick of a wedge episode, and forced-pause enforcement; `cost-tracker`'s budget alerts (80%/100% caps) push directly instead of only setting a flag for the next heartbeat wake, falling back to that flag on a failed send. A new `channel-status-responder.ts` hook answers an exact idle `"status"` channel message (session status, pause state, budget-vs-cap, pending approvals, next routine) at zero model cost — send-then-block, so a failed send always falls through to a normal model turn instead of going silent. Every send is logged to the episodic channel-log (`direction:"out"`) same as a model-sent reply.
...
- No config toggle: the pause mechanism is inert by construction (`state/pause.json` absent = fully unpaused, identical to pre-1.2.17 behavior) until an operator or an authorized channel sender invokes it — there is nothing to opt into or out of.
