...
**Session-lifecycle permission migration.** Re-run `bun ${CLAUDE_PLUGIN_ROOT}/scripts/apply-settings.ts <hatch_target file> allow` (the same settings file `hatch` originally wrote to — `.claude/settings.json` or `.claude/settings.local.json`, per this hermit's `hatch_target`) to add the new `Bash(bun */scripts/session-archive.ts*)` entry to the sealed allow-list. This is additive and idempotent (`mergeAllow` dedups by exact string) — safe to run even if the entry is already present. Without it, the hermit is asked for permission (functionally denied in headless/channel sessions) on its first post-upgrade idle transition.
