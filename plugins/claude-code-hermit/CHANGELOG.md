...
- Peer messages from a `bypassPermissions` session reach the hermit instead of stalling it on an approval dialog until the message expires. The launch overlay carries `crossSessionInbound: "accept"`, the only scope that can loosen the key; a value in your own `~/.claude/settings.json` still wins, and a project- or local-scope `"refuse"` still opts out.
