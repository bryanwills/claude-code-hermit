...
### Changed
- `proposal-act` keeps its guards, accept steps and dispatch in `SKILL.md` and reads each action's procedure from `branches.md`; `hermit-settings` states the write rule before the per-setting branches
