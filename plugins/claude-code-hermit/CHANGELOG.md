...
- `apply-settings.ts`'s `output-style` op now checks both project scopes (`.claude/settings.local.json` and `.claude/settings.json`) before seeding, not just its target file, and takes an optional style argument validated against Claude Code's known built-ins plus `hermit-voice`. A new `output-style-set` op does the explicit replacement `hermit-settings voice` needs; it is deliberately outside the auto-mode classifier's preauthorized op list, reachable only from a live terminal choice.
...
- `hermit-doctor`'s `voice-carrier` check only ever looked at local and project settings, so a style set at user scope (`~/.claude/settings.json` or a relocated `CLAUDE_CONFIG_DIR`) was invisible to it and the check reported "using Claude Code defaults" for an install that had one. Seeding still stops at project scope on purpose: both project scopes outrank user scope, so a value there can never shadow the write and never blocks it.
