...
- `manifest-seed.ts` and `reflect-precheck.ts` now pin their state-dir argument the same way as `proposal.ts`; `reflect-precheck.ts`'s pin also covers `archive-shell.ts`, `archive-raw.ts` and `update-reflection-state.ts`, which it forwards the argument to as a subprocess call.
- `update-reflection-state.ts` now requires its file-path argument to resolve inside the current project's state dir. Its argument was a file, not a directory, so it was the least bounded of the pre-approved scripts: an unvalidated call could write arbitrary JSON at arbitrary depth on disk, not just inside a hermit's own state.
- `generate-summary.ts`'s CLI mode (used by hatch, docker-setup, and hermit-evolve to seed a manifest) now requires its state-dir argument to resolve inside the current project's state dir; its PostToolUse hook mode is unaffected.
