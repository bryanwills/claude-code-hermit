...
- The hermit boots with `--name <agent_name>` on Claude Code 2.1.224+, sanitized to `[A-Za-z0-9_-]` and falling back to the tmux session name, so other local sessions can address it with `ListAgents`/`SendMessage` across restarts. The registered name (or `null` when the flag was skipped) is recorded in `state/runtime.json` as `peer_name`.
