...
- **Hermit Dashboard artifact** — new `config.artifacts.dashboard` flag publishes a single persistent [Artifact](https://code.claude.com/docs/en/artifacts) page (status, proposal queue, weekly evolution) to a private `claude.ai/code/artifact/<uuid>` URL, republished to the same address by `brief`, `weekly-review`, `proposal-create`, and `proposal-act`. Rendered by a new deterministic script (`scripts/lib/dashboard.ts` + `render-dashboard.ts` CLI) — no model authorship, so a refresh costs a render, not a generation — and gated by a content hash so an unchanged page never mints a new artifact version. Publish failures (no entitlement, tool absent) fall back silently to the existing markdown-only channel delivery; never blocks or degrades it. Ships **disabled** — entitlement-gated (Pro/Max/Team/Enterprise, `/login`-authenticated) and publishes session-derived content to claude.ai, the same opt-in bar as `telemetry_export`. See `docs/artifacts.md` for the refresh protocol and `docs/config-reference.md#artifacts` for privacy/entitlement notes.
...
- No config toggle for pause: the mechanism is inert by construction until an operator or an authorized sender invokes it. The new pause files (`operator-pause.json`/`auto-pause.json`), the alert-state split files (`budget-alerts.json`/`telemetry-alert.json`), and `channel-health.json` are all created lazily by the shipped scripts on first write — no migration. A pause in force in the legacy `state/pause.json` at upgrade time is still honored until the operator resumes.
...
- Existing hermits are missing the `artifacts` config block entirely. The same generic `new_config_keys` template sync adds `"artifacts": {"dashboard": false}` verbatim — inert, opt-in, nothing to migrate. The `## Dashboard Refresh` pointer in `CLAUDE-APPEND.md` ships as part of the existing Step 6 whole-block sync, so a normal `/claude-code-hermit:hermit-evolve` picks it up alongside the config key; no separate bespoke step. Enable via `/claude-code-hermit:hermit-settings artifact-dashboard`.
