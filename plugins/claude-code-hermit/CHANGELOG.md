...
- `config.effort` — passed as `--effort` on every `hermit-start`, mirroring `config.model`. Without it a channel effort change would persist instead of reverting on restart. Not the same lever as `config.env.CLAUDE_CODE_EFFORT_LEVEL`, which pins the session and makes a runtime `/effort` a no-op.

### Fixed
- A `/clear` or `/compact` reaching the pane outside the watchdog no longer skips the hermit's own reset bookkeeping. The runtime stamp, `SHELL.md` breadcrumb, and status-cache clear moved to `lib/context-reset.ts`; skipping the cache clear previously let the watchdog fire a spurious `/compact` against a freshly-cleared context.
