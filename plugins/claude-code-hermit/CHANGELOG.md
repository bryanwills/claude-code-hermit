...
### Fixed
- `rc-server gc` collects orphaned bridge worktrees on macOS. `liveCwds()` read `/proc`, which does not exist there, so it returned null and the GC refused to collect anything; it now reads process cwds through `lsof` on darwin, bounded at 5s. Any failure still returns null rather than an empty set, so the GC skips instead of treating every worktree as unused and deleting live ones.
- Concurrent `PermissionDenied` hook processes no longer lose each other's writes. Claude Code does not serialise hook invocations, so parallel tool calls in one turn spawn overlapping hooks; both the denial record and the dedup window are now written under one bounded advisory lock, the same one `SHELL.md` appends use. Previously a burst could drop a suppressed count and under-report `(+N more)`, and the record relied on the filesystem serialising concurrent appends — which a bind-mounted tree under Docker Desktop for macOS does not guarantee.
...
- `reflect`'s `skill-correction:*` and `skill-preference:*` plugin-skill branches no longer fire on an unreadable list, so a candidate stops asserting a name is plugin-shipped on no evidence; procedure-capture verification now cites the probed next-user-turn skill visibility instead of a session reload.
