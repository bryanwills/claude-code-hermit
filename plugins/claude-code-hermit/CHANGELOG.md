...
- Security-tier settings are gated by which chat asks: a new `PreToolUse` gate (`channel-settings-gate.ts`) tiers every settings write by the chat id on the turn's opening message. An arbitrary chat can still only change safe settings; the chat configured as `maintainer_channel_id` can also change boot skill, remote, escalation, Docker and the artifact backend. It keys on the turn, not the session, so typing in the hermit's own terminal is unaffected.
- Permission mode, `env` and `monitors` need a confirmation code. From the maintainer chat the hermit posts a short code and applies the change only after you echo it back in a second message — a code is single-use, tied to one setting and one chat, and expires in 10 minutes. These three reach what the session may execute, so the chat's authority alone doesn't settle them.
- Channel enrollment stays terminal-only on every chat: `allowed_users`, `default_chat_id`, `dm_channel_id` and `maintainer_channel_id` — plus any parent write that would replace them, and direct `config.json` edits — change only from a terminal. A maintainer chat that could re-point itself or add a user would make one compromise permanent instead of something you can revoke.
