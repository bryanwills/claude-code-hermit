...
- **reflect: routine ROI signal** — extends the routine-health check with a channel-engagement join: reads `channel-replies.jsonl`, computes an until-next-send engagement ratio per routine, joins per-routine cost from `cost-log.jsonl`, and proposes Tier-1 disable or re-time when a channel-delivering routine has ≥10 fires and ≤20% engagement.
