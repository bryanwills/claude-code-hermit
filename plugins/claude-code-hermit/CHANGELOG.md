...
### Fixed
- A queued daily close that fails to complete now retries on the next heartbeat tick instead of skipping one. The drain's 30-minute backoff was tuned for the 60-second routine poll; at the current `heartbeat.every` of `30m` it could never expire in time, so a heartbeat-only hermit retried roughly hourly. The heartbeat drain now caps its backoff at half its own interval.
