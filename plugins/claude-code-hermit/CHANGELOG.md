...
- `/claude-code-hermit:channel-setup` fills a missing channel `state_dir` through `hatch-config --reinit` rather than writing the key directly, so that path is validated and audited like every other channel write.
