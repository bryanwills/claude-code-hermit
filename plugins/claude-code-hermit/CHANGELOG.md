...
- The `## Skill Improvement` gate now treats an available-skills list not in context as unknown, proceeding to the confirmation-gated override path, and rejects `stale-paths` only on a readable list; procedure-capture verification and reflect routing now follow the probed next-turn visibility.
