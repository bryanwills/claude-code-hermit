...
- The watchdog's heartbeat-staleness threshold is now `max(stale_factor × heartbeat.every, 4h)`. The nudge it gates is a paid full-context wake, so its cadence is recovery policy rather than a multiple of the poll interval; without the floor the new default would have quadrupled the probe rate on a quiet open session arc. `stale_factor` still widens the threshold past the floor, but the floor also applies downward: an install running `heartbeat.every` under `2h` no longer gets a wedge threshold tighter than 4h — raise `stale_factor` if you want it wider, and expect nudges no sooner than 4h either way.
