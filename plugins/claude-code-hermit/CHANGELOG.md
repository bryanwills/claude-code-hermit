...
- **NEXT-TASK numbered-bullet append simplified (review pass).** Replaced the brittle "pick `4.` or `5.` depending on whether the prior bullet was appended" instruction with "append in order, numbered sequentially from `4.`" plus `(if ...)` prefixes on each conditional bullet. The quality-gate condition now references step e.5 instead of restating the truthiness clause, keeping a single source of truth and trimming prompt tokens.
