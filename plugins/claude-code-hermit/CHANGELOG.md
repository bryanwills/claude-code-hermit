...
- **Artifact publish authorization.** If any `artifacts.*` flag ends up `true` after the sync above, offer the operator: "This hermit publishes status/proposal/weekly-review pages via Claude Code's Artifact tool. Unattended sessions can't answer a permission prompt, so authorize it now, or publish the first version of each page yourself right now instead?" On accept (default), run `bun ${CLAUDE_PLUGIN_ROOT}/scripts/apply-settings.ts <hatch_target settings file> artifact-allow`. On decline, publish the first version of each enabled page inline per `docs/artifacts.md`'s refresh procedure, recording each URL in `state/artifacts.json` so later refreshes redeploy to the same URL without prompting. This is a genuine either/or with no safe default — unattended evolve defers it to the channel via the existing deferred-migration mechanism (SKILL.md Step 10) rather than guessing.
