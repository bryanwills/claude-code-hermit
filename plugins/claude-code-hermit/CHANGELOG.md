...
### Changed
- A proactive notice always runs `channel-send --notice`, including on a channel-less install, and falls back to `PushNotification` on `no_channel: true`. The result carries `channels_configured` so an unreachable configured channel still raises a `channel-send-unavailable` issue while an intentionally empty channels block stays quiet.
