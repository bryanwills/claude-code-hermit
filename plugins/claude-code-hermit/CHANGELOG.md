...
- Cost rows carry `observed_at` (when the context was seen, not when the row was written) and `last_call_prompt_tokens` (the turn's newest call, correct for a turn that compacted mid-flight, where `max_prompt_tokens` still reports the discarded peak).
