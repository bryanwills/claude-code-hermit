...
- A command addressed to the hermit's own bot handle (`/pause@yourbot`) is accepted on any channel with a stored `bot_username`, and one addressed to a different bot is ignored. Fixes Telegram group chats, where the client rewrites a command picked from the bot menu and it previously did nothing.
