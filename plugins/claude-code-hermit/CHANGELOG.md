...
5. **`permission_mode: "bypassPermissions"` hermits: the same restart is required, not optional.** That mode holds an incoming peer message behind an approval dialog and drops it after five minutes, so the socket wake would fail silently. The restart is what writes `crossSessionInbound: "accept"` into `.claude/settings.local.json`. To keep the old behaviour instead, leave the key out and remove it after each boot, or switch `permission_mode` to `auto`.
