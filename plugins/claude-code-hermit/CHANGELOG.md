...
### Fixed
- The heartbeat's clean-recheck damper now arms as soon as an alert is recorded, not after five fires. An already-recorded alert firing again sends nothing, so each repeat cost a full evaluation for news the operator already had; on a 30m heartbeat with the default 6h cooldown that is one wake per window instead of one per tick.
