...
### Added
- `channels.<name>.settings_policy` (`allow` | `ask` | `deny`, terminal-only) decides how far chat-side settings authority reaches on that channel: `allow` applies every chat-reachable tier on the first message, `ask` keeps the echoed confirmation code for execution-adjacent settings, `deny` keeps everything above the everyday settings terminal-only there. New channel entries ship `allow`; an absent or unrecognised value reads as `ask`. Edit it from a terminal with `/claude-code-hermit:hermit-settings channels` → `edit <name>` → `settings_policy`.
...
- `settings_from_chat` is retired and no longer read. Its `false` becomes `settings_policy: deny` on every channel; `hermit-evolve` migrates it, and a leftover key warns instead of failing validation.
...
**Migrate `settings_from_chat` to per-channel `settings_policy`.** Read the current value with `bun "${CLAUDE_PLUGIN_ROOT}/scripts/settings-edit.ts" .claude-code-hermit/config.json get settings_from_chat`, and the channels with `… get channels`.

1. For each key under `channels` except `primary` that has no `settings_policy` yet, decide its value: `deny` if `settings_from_chat` was `false`; else `ask` if that entry's `allowed_users` is an array naming two or more ids; else `allow`.
...
3. Remove the retired key: `.claude-code-hermit/bin/hermit-run settings-edit .claude-code-hermit/config.json unset settings_from_chat`.
