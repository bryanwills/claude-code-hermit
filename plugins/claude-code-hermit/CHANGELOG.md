...
### Fixed
- `state/state-summary.md` no longer reports stale alert counts after a change confined to `budget-alerts.json` or `telemetry-alert.json`. The hand-maintained mtime fast path is gone; the content-equality check before the write is now the sole redundant-write guard, and its own line-skip bug (which had kept the `updated:` stamp in the comparison, so it never matched) is fixed too.
