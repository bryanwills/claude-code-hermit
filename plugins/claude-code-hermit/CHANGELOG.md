...
- The auto-mode grant for the hermit's own settings maintenance is narrower. It now names a concrete install prefix instead of a `*/scripts/` glob, spells out the exact command and argument shape, enumerates the settings ops and target files it covers, and excludes `.mcp.json`, hooks, approval gates and `config.json` outright. It is void if the session authored the script itself or would re-achieve a settings change already blocked. An upgrade or migration can still trigger the run unattended, but can no longer dictate which script, op or file it touches.
