...
### Fixed
- The watchdog's lapsed-login tiers read the session's own auth environment instead of their own. On a host install the watchdog runs from a unit carrying only `PATH`, so a token hermit with a custom `CLAUDE_CONFIG_DIR` was told to run `/login` by hand instead of the re-auth relay renewing it, and an API-key hermit's `401` was reported as an expired login and then went silent. The session now records `config_dir` and `env_auth` in `state/runtime.json` at startup, and the watchdog prefers them, falling back to its own environment when they are absent.
