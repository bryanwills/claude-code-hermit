...
`auth_mode` is resolved from the credential volume when it is unset, so a hermit that predates the key behaves exactly as it did. On macOS a login lives in the Keychain, which this code cannot inspect; such an install resolves as `external` and sits every renewal path out. So does an API key or cloud-provider credential in the environment — no relay can renew what the operator's own shell owns.
