...
Yes. Docker is the guided always-on path, not a requirement. Use `hermit-start`/`hermit-stop` for bare tmux; the first always-on boot registers the watchdog scheduler so dead sessions come back (opt out with `watchdog.scheduler_enabled: false` or `bin/hermit-watchdog uninstall`). For interactive-only use, just run `/claude-code-hermit:session`. No tmux or Docker needed.
