...
**Session report** (`sessions/S-NNN-REPORT.md`): `id`, `status`, `date`, `duration`, `cost_usd`, `tags`, `proposals_created`, `task`, `escalation`. Optional: `closed_via` (`operator` | `auto`; absent in legacy reports — treat as `operator`).
