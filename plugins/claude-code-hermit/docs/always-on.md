...
| ---- | -------------- | ------------------- | ----------------- |
| apt package | `docker.packages` via `/claude-code-hermit:hermit-settings docker` | at the next template render: `/docker-setup`, or the next `hermit-evolve` (a rebuild alone does not re-render). Need it this session: use the sidecar row | survives every upgrade, it lives in `config.json` |
