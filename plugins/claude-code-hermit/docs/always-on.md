...
| ---- | -------------- | ------------------- | ----------------- |
| apt package | a `RUN apt-get` layer in `Dockerfile.hermit` (`docker.packages` in `config.json` is read only at render time, so setting it installs nothing on its own) | rebuild on the host: `.claude-code-hermit/bin/hermit-docker up --build`. The package layer sits above the Claude Code install, so the rebuild also reinstalls Claude Code at the latest version | a later upstream change to the file arrives as a 3-way merge, with your edit re-applied and the previous copy parked under `.claude-code-hermit/state/`, rather than as a straight overwrite. Re-check it after an upgrade |
...
| volume, port, capability, base image | one contiguous commented block in `docker-compose.hermit.yml` or `Dockerfile.hermit` | `.claude-code-hermit/bin/hermit-docker up --build` or `.claude-code-hermit/bin/hermit-docker restart`, as the change requires | `hermit-evolve` 3-way merges from the `docker-setup` baseline when there is one and upstream moved, parking the previous copy at `.claude-code-hermit/state/<name>.<timestamp>.bak`; otherwise it leaves the file alone. Re-check the block after every evolve |
