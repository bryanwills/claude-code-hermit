...
| ---- | -------------- | ------------------- | ----------------- |
| apt package | a `RUN apt-get` layer in `Dockerfile.hermit` (`docker.packages` in `config.json` is read only at render time, so setting it installs nothing on its own) | rebuild on the host: `.claude-code-hermit/bin/hermit-docker update` | the edit marks the file customized, so `hermit-evolve` keeps your version and stops applying upstream changes to it. Re-check it after an upgrade |
