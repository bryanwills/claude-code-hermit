...
| ---- | -------------- | ------------------- | ----------------- |
| apt package | `docker.packages` via `/claude-code-hermit:hermit-settings docker`, **and** the same list written into the `# Project-specific packages` layer of `Dockerfile.hermit` (config is read at render time, not at build time) | rebuild on the host from the project root: `docker compose -f docker-compose.hermit.yml build` | `config.json` keeps the list; matching the rendered shape keeps the next `hermit-evolve` merge clean |
