...
- **The Hermit Dashboard publishes session-derived content to claude.ai — opt-in, off by default.** `config.artifacts.dashboard` (default `false`) publishes a rendered status/proposal/weekly-review page via Claude Code's built-in Artifacts feature, not a hermit-authored HTTP call — the same publish path as any other Artifact a session creates, so it's governed by Claude Code's own controls (`disableArtifact`, `CLAUDE_CODE_DISABLE_ARTIFACT`, `permissions.deny`), not the docker-security DNS allowlist. Private to the publishing account on Pro/Max; Team/Enterprise can additionally Share within the org, never across it. See `docs/config-reference.md#artifacts` and `docs/artifacts.md`.
