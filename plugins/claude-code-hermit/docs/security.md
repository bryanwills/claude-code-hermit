...
The sandbox wraps every `Bash` tool call with `bwrap` (Linux) or the platform equivalent. If the probe fails (bubblewrap unavailable), `/hatch` prints an install hint and skips — no silent fallback to an unsandboxed state.

**Opting out:** set `sandbox.enabled: false` in your settings file.

**Docker:** the hermit image ships `bubblewrap` and `socat`; `hermit-start` auto-sets `enableWeakerNestedSandbox: true` inside the container (required for unprivileged nested sandboxing).
