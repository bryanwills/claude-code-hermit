...
**Docker / Ubuntu 24.04+ hosts:** on hosts with `kernel.apparmor_restrict_unprivileged_userns=1` (Ubuntu 23.10+ default), the AppArmor `docker-default` policy blocks unprivileged user-namespace creation inside the container. `bwrap` cannot start, so `hermit-start` detects this at boot and auto-disables the nested sandbox (`sandbox.enabled:false` in `.claude/settings.local.json`). This lets heartbeat and `/watch` Monitor subprocesses spawn; the container boundary (`cap_drop:ALL`, `no-new-privileges`, network policy) is the isolation layer — the Anthropic-recommended posture (https://code.claude.com/docs/en/sandboxing). Note: `sandbox.enableWeakerNestedSandbox` does NOT bypass a blocked user-namespace — it only addresses the `/proc` access case in capable containers.

**Opting out:** set `sandbox.enabled: false` in your settings file.

**Re-enabling after fixing host AppArmor:** set `sandbox.enabled:true` and delete `.claude-code-hermit/state/sandbox-probe.json` to clear the cached probe result.
