...
| `stale_factor` | number | `2` | Heartbeat staleness multiplier. Session is considered stale when `now - mtime(state/.heartbeat) > max(stale_factor × heartbeat.every, 4h)`. The 4h floor keeps the nudge probe — a paid full-context wake — at a fixed recovery cadence regardless of how short the poll interval is; raise `stale_factor` to widen past it. The floor is a hard lower bound in both directions: with `every` below `2h` the product no longer applies, so a short interval cannot tighten wedge detection under 4h. |
