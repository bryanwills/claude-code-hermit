...
| `bot_user_id` | string | _(absent)_ | The hermit's **own** bot account id on this platform, captured at pairing by `/channel-setup` (or backfilled by `hermit-evolve`). Lets the reply reminder tell the model that a mention of this id is addressed to it — without it, a mention-gated channel can deliver `<@bot-id>` and the model reads it as third-party traffic. Never an auth input. Re-running `/channel-setup` refreshes it after a bot swap; `hermit-doctor` warns when it no longer matches the token's bot. |
| `bot_username` | string | _(absent)_ | The bot's own `@handle`, captured alongside `bot_user_id`. Telegram mentions carry the handle rather than the numeric id, so the self-mention match needs both. |
