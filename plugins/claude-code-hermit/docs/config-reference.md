...
| `scope` | string | `"local"` | Git scope: `"project"` tracks hermit state in git (sessions, proposals, config — recommended for solo projects); `"local"` gitignores hermit state (recommended for teams). Determines which `.gitignore` template hatch stamps. Run `/migrate` before committing in project scope to catch unprotected credentials. |
