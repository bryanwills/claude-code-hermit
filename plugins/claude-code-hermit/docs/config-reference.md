...
| `maintainer_channel_id` | string | _(absent)_ | Optional second destination (same bot/token, a different chat) for technical, operational, and spend content, and the one chat that may change the settings tier of `config.json` from a channel (`docs/security.md` § Tiered settings authority) — `boot_skill`, `remote`, `escalation`, `docker.*`, `artifacts.backend` directly, `permission_mode`/`env` with an additional echoed confirmation code, allowlist-checked either way. When set, tiered alerts route here instead of the primary chat; the primary chat sees only plain-language client copy. Reached by a direct platform API POST with the bot token, not `access.json` — the bot must be present/permissioned in the target chat, no channel-plugin authorization is involved. Not a *control* surface: control keywords and reauth replies still come through the primary DM chat. Set manually. Verify reachability with `/channel-setup` or `/smoke-test`, since a typo'd id otherwise fails silently on every alert. |
