...
**Steering message:** the `/compact` call carries one of two fixed prose messages, chosen by `session_state` at the moment the compact actually fires — not by the marker above (which can be up to 1h old and only waives the interval cooldown). `session_state === 'idle'` gets a message that keeps pending operator items and drops completed-arc detail; anything else (mid-arc, or the field absent) keeps the existing focus-on-unfinished-work message.
