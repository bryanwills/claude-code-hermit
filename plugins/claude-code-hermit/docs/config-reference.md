...
| `operator_profile` | string | `"technical"` | Audience of the person on the primary channel: `"technical"` (the maintainer runs the box, so technical/ops/spend detail may reach the primary chat) or `"non-technical"` (a client/end-user, not the maintainer). `non-technical` forces technical alerts and spend figures to a channel's [`maintainer_channel_id`](#channels) (or SHELL.md Findings when none is set) and deflects client-chat spend questions with a plain "handled by your provider" reply. |
...
| `stale_factor` | number | `2` | Heartbeat staleness multiplier. Session is considered stale when `now - mtime(state/.heartbeat) > max(stale_factor × heartbeat.every, wedge_floor)`. Raise it to widen the threshold past the floor; to tighten below the floor, lower `wedge_floor` instead. |
