...
A midnight-adjacency guard suppresses compaction when the `daily-auto-close` routine is due to fire within the next 10 minutes (the same auto-close lull `post_close_clear` waits out) and `post_close_clear` is on — that `/clear` will reset the context for free shortly after, so a compact right before it would be a wasted summarization call. The guard is deliberately narrow: a wider window would blank the compact tier across the whole evening operator slot, when context grows fastest. With `post_close_clear: false` no free reset is coming, so the guard never applies.
