...
| `settings_from_chat` | boolean | `true` | See [`settings_from_chat`](#settings_from_chat) below. |
...
| `maintainer_channel_id` | string | _(absent)_ | Optional second destination (same bot/token, a different chat) for technical, operational, and spend content, and the one chat that may change the settings tier of `config.json` from a channel (`docs/security.md` § Tiered settings authority) — `remote`, `escalation`, `docker.*`, `artifacts.backend` directly, `permission_mode`/`env`/`monitors`/`boot_skill` with an additional echoed confirmation code, allowlist-checked either way. When set, tiered alerts route here instead of the primary chat; the primary chat sees only plain-language client copy. Reached by a direct platform API POST with the bot token, not `access.json` — the bot must be present/permissioned in the target chat, no channel-plugin authorization is involved. Not a *control* surface: control keywords and reauth replies still come through the primary DM chat. Set manually. Verify reachability with `/channel-setup` or `/smoke-test`, since a typo'd id otherwise fails silently on every alert. |
| `bot_user_id` | string | _(absent)_ | The hermit's **own** bot account id on this platform, captured at pairing by `/channel-setup` (or backfilled by `hermit-evolve`). Lets the reply reminder tell the model that a mention of this id is addressed to it — without it, a mention-gated channel can deliver `<@bot-id>` and the model reads it as third-party traffic. Never an auth input — it is only ever matched against message text. Automatic capture covers Discord and Telegram; any other channel (iMessage, third-party plugins) prints a `SKIP` line instead, and setting the key by hand there gives the same behavior. Re-running `/channel-setup` refreshes it after a bot swap; `hermit-doctor` warns when it no longer matches the token's bot. |
| `bot_username` | string | _(absent)_ | The bot's own `@handle`, captured alongside `bot_user_id`. Telegram mentions carry the handle rather than the numeric id, so the self-mention match needs both. Unlike the id, a handle can be reassigned — `hermit-doctor` warns when the bot has been renamed, because a stale handle stops matching and can start matching whoever claimed the freed name. |
...
- **execution-adjacent** (`permission_mode`, `env`, `monitors`, `boot_skill`) — the settings chat *plus* a confirmation code the hermit posts and the operator echoes back within 10 minutes. The ask names the change alongside the code, with credential values shown as `[set]`/`[cleared]` rather than repeated back. `boot_skill` sits here because it becomes the boot prompt of every session the hermit starts: standing instructions, re-applied on every restart, unattended.
...
Set to `false` to collapse everything above the everyday tier to terminal-only on **every** chat, the settings chat included; the everyday tier and reads are unaffected. Terminal-only to change (a chat that could re-arm it could undo the decision), and validated by `validate-config.ts` — only a literal `false` opts out.
