...
| `effort` | string \| null | `null` | Override reasoning effort, passed as `--effort` at boot (e.g. `"high"`). Re-asserted on every start, so a channel `/effort` change reverts on restart. Leave `null` to use the model default. Mutually exclusive in practice with `env.CLAUDE_CODE_EFFORT_LEVEL`, which pins the session and makes a runtime `/effort` do nothing. |
