...
| `operator_profile` | string | `"technical"` | Audience of the person on the primary channel: `"technical"` (the maintainer runs the box, so technical/ops/spend detail may reach the primary chat) or `"non-technical"` (a client/end-user, not the maintainer). `non-technical` forces technical alerts and spend figures to a channel's [`maintainer_channel_id`](#channels) (or SHELL.md Findings when none is set) and deflects client-chat spend questions with a plain "handled by your provider" reply. It also decides **settings authority**: on a `technical` install with no `maintainer_channel_id`, the hermit's own home chat carries the security tier; `non-technical` keeps that tier terminal-only until a maintainer chat is configured. Because it grants authority, it changes only from a terminal session — see [`settings_policy`](#settings_policy) and [`docs/security.md`](security.md) § Tiered settings authority. |
| `settings_permissions` | object | absent | Re-tiers which settings a chat may change, in Claude Code's own `allow`/`ask`/`deny` rule shape. Absent means the built-in tiers apply unchanged. Cannot reach the channel enrollment root, `operator_profile`, or itself. Terminal-only to change — see [`settings_permissions`](#settings_permissions). |
| `channels` | object | `{}` | Channel configuration keyed by channel name. See [`channels`](#channels) section. |
| `remote` | boolean | `true` | Enable Remote Control via claude.ai/code. When `false`, boot sets `isolatePeerMachines: true`, so cross-machine peer messages require approval. |
...
- **security tier** (`remote`, `escalation`, `docker.*`, `artifacts.backend`, and anything not otherwise listed) — the **settings chat**: the configured [`maintainer_channel_id`](#channels), or the home chat itself when none is configured and [`operator_profile`](#top-level-keys) is `"technical"`.
- **execution-adjacent** (`permission_mode`, `env`, `monitors`, `boot_skill`, `shutdown_skill`) — the settings chat, *plus* a confirmation code the hermit posts and the operator echoes back within 10 minutes when the channel's `settings_policy` is `"ask"`. The ask names the change alongside the code, with credential values shown as `[set]`/`[cleared]` rather than repeated back. The two skill hooks sit here because they run unattended on every launch and every full close: `boot_skill` becomes the boot prompt of every session the hermit starts, and `shutdown_skill` the teardown command each close invokes.
- **never from chat** — `allowed_users`, `default_chat_id`, `dm_channel_id`, `maintainer_channel_id`, `settings_policy`, `operator_profile`, `settings_permissions`, and any direct `config.json` write.

Which path sits on which tier is the default, not the law — [`settings_permissions`](#settings_permissions) re-tiers any of them.

```json
"channels": { "discord": { "settings_policy": "ask" } }
```

What each value does to the tiers above:

| value | effect on that channel |
|---|---|
| `allow` | the settings chat applies every chat-reachable tier on the first message — no confirmation code. The default for a new channel entry: a freshly paired channel is one operator on their own chat, where the code has nobody to separate. |
| `ask` | execution-adjacent settings need the echoed code. Use it when more than one person can post in the settings chat. The fallback for an absent or unrecognised value. |
| `deny` | everything above the everyday tier is terminal-only on that channel, the settings chat included; the everyday tier and reads are unaffected. |

Terminal-only to change (a chat that could raise its own policy would drop its own second factor), and validated by `validate-config.ts`: an unrecognised value is rejected rather than coerced, and `"allow"` on a channel whose `allowed_users` names several people warns.

---

## `settings_permissions`

Top-level. The tier table above is this hermit's default, not its law: `settings_permissions` moves any setting to a different tier, in either direction, without forking the gate script. Same shape and vocabulary as Claude Code's own [`permissions`](https://code.claude.com/docs/en/settings) key, because it answers the same question about the same kind of subject.

```json
"settings_permissions": {
  "allow": ["routines", "routines.*.precheck"],
  "ask":   ["model"],
  "deny":  ["boot_skill"]
}
```

| list | tier it moves a path to |
|---|---|
| `allow` | everyday — the operator's own chat, first message, no code |
| `ask` | the settings chat **plus** the confirmation code (the channel's `settings_policy` still decides whether the code is actually asked for) |
| `deny` | terminal-only |
...
Terminal-only to change, for the same reason `settings_policy` is.
...
**Guards** (same shape as `watchdog.context_clear_tokens`): always-on only (never interactive sessions), no in-flight transition, operator silent ≥ 10 min, session quiescent (pane unchanged across two consecutive scheduler ticks — tracked independently of the `/clear` mechanism's quiescence state), and never within the same tick as an emergency `/clear` (the 700k path takes precedence). A hard floor of 60,000 tokens applies regardless of configuration — a context that small is never worth summarizing. Both mechanisms judge the resident's own context and nothing else: they resolve `runtime.json`'s `cc_session_id` (the resident's Claude Code session id, stamped by the SessionStart hook under `HERMIT_MANAGED`) and read only cost-log rows whose `cc_session_id` matches and which are not marked `guest`. With no `cc_session_id` recorded, both tiers skip rather than guess. A hatched folder can hold several sessions at once — worktree guests, a hand-launched maintenance session — and the `session_id` arc label is shared across all of them while an arc is open, so it identifies no one.
...
Change it with `/claude-code-hermit:hermit-settings voice`, from a terminal or from your chat. The style name is an everyday setting; custom prose is execution-adjacent (it becomes every future session's system prompt), so on an `ask` channel it takes the confirmation code.
