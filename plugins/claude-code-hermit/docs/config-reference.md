...
| `dm_channel_id` | string | `null` | DM channel ID learned from the first inbound message. Used for proactive outbound notifications. Never set manually. A `chat_id` equal to the channel's `maintainer_channel_id` is never learned here — the maintainer chat stays outbound-only. |
