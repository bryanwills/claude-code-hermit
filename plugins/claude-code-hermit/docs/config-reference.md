...
Out-of-session supervisor that detects dead or wedged sessions and restarts them. On a tmux always-on boot, `hermit-start` registers the OS scheduler automatically (`scheduler_enabled` defaults to `true`). That first registration also sets `enabled: true`; hygiene-only (`enabled: false` with the timer installed) is still valid. Opt out with `scheduler_enabled: false` or `bin/hermit-watchdog uninstall`. **Docker hermits:** `/docker-setup` sets `enabled: true` automatically at the end of setup; the entrypoint loop already runs the watchdog, so no host scheduler is needed.

| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `scheduler_enabled` | boolean | `true` | Durable scheduler policy. `true` (the default, including when the key is absent) means a tmux always-on boot will run `bin/hermit-watchdog install`. `false` is the operator opt-out: boot skips registration, and doctor reports "scheduler opted out" instead of telling you to install. `install` sets this `true`; `uninstall` sets it `false`. Distinct from `enabled` (recovery policy) so a hygiene-only hermit can keep the timer without restarts. |
