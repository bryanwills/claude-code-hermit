...
| `language` | string | `null` | Preferred language for responses. Auto-detected from locale. Also selects the language of script-composed channel messages (budget, denial, mint, watchdog, status): any Portuguese signifier (`pt`, `pt-*`, `portuguese`, `português`) resolves to `pt-PT`; anything else to `en`. |
...
| `maintainer_channel_id` | string | _(absent)_ | Optional outbound-only second destination (same bot/token, a different chat) for technical, operational, and spend content. When set, tiered alerts route here instead of the primary chat; the primary chat sees only plain-language client copy. Not an inbound surface: control keywords and reauth replies still come through the primary DM chat. Set manually. Verify reachability with `/channel-setup` or `/smoke-test`, since a typo'd id otherwise fails silently on every alert. |
