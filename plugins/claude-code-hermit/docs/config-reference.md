...
| `peer_notices` | object | `{"enabled": true, "max_idle_minutes": 30}` | Supplementary local notice delivery to one Claude Code session. The freshest state transition wins, and `max_idle_minutes` bounds the age of that last state transition (not last activity). Hermit residents and `bg` sessions are never targets. A `bypassPermissions` receiver holds the post behind a dialog that expires after five minutes. Set `enabled: false` to disable. |
...
| `maintainer_channel_id` | string | _(absent)_ | Optional second destination (same bot/token, a different chat) for technical, operational, and spend content, and the one chat that may change the settings tier of `config.json` from a channel (`docs/security.md` § Tiered settings authority) — `remote`, `escalation`, `docker.*`, `artifacts.backend` directly, `permission_mode`/`env`/`monitors`/`boot_skill`/`shutdown_skill` with an additional echoed confirmation code, allowlist-checked either way. When set, tiered alerts route here instead of the primary chat; the primary chat sees only plain-language client copy. Reached by a direct platform API POST with the bot token, not `access.json` — the bot must be present/permissioned in the target chat, no channel-plugin authorization is involved. Not a *control* surface: control keywords and reauth replies still come through the primary DM chat. Set manually. Verify reachability with `/channel-setup`, since a typo'd id otherwise fails silently on every alert. |
