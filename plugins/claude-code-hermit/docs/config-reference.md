...
| `maintainer_channel_id` | string | _(absent)_ | Optional second destination (same bot/token, a different chat) for technical, operational, and spend content, and the one chat that may change the settings tier of `config.json` from a channel (`docs/security.md` § Tiered settings authority) — `remote`, `escalation`, `docker.*`, `artifacts.backend` directly, `permission_mode`/`env`/`monitors`/`boot_skill`/`shutdown_skill` with an additional echoed confirmation code, allowlist-checked either way. When set, tiered alerts route here instead of the primary chat; the primary chat sees only plain-language client copy. Reached by a direct platform API POST with the bot token, not `access.json` — the bot must be present/permissioned in the target chat, no channel-plugin authorization is involved. Not a *control* surface: control keywords and reauth replies still come through the primary DM chat. Set manually. Verify reachability with `/channel-setup` or `/smoke-test`, since a typo'd id otherwise fails silently on every alert. |
...
7a. If the pane shows an auth failure: skip the lapsed-login tiers entirely when `env_auth` says the session's environment owns the credential (an API key, a bearer token, or Bedrock/Vertex/Foundry) — a 401 there is not a login lapse and must not silence the tiers below. Otherwise, a token-mode hermit (per the stamped `config_dir`) spawns the re-auth relay, and a `/login` hermit gets a `lapsed-login-detected` notice repeating daily.
...
- **The rendered voice file is gitignored**, so it doesn't travel with a clone — `/claude-code-hermit:migrate` treats it as must-migrate and it's in `.worktreeinclude`. Migrating `config.json` is enough on its own: the next boot re-renders it.
