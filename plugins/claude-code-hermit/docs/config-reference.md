...
| `dm_channel_id` | string | `null` | DM channel ID learned from inbound messages. Used for proactive outbound notifications. Never set manually. Only a reply the hermit sends during a turn that an inbound message from that same chat opened is learned here; a proactive send (routine, heartbeat, scheduled brief) into a second chat never moves it. A `chat_id` equal to the channel's `maintainer_channel_id` is never learned here either — the maintainer chat stays outbound-only. |
