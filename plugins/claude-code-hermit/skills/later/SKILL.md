...
Call `later add .claude-code-hermit --claim <text> [--cmd <shell>] --due <ISO> --origin operator|hermit`, with `--session T-...` when the current session is known and `--timeout-s <1-300>` only when the evidence command is known to need more than the 30-second default. On every channel turn, including home chats and open task threads, pass the asking chat as `--chat <sourceKey>:<chat_id>`. Quote each argument literally. Origin is `operator` when the operator typed the request, and `hermit` when you reached for this skill yourself, including workflow pointers.
...
`list` calls `later list .claude-code-hermit --chat <sourceKey>:<chat_id>` from a channel turn, scoped to the asking chat. From the terminal, omit `--chat`. Honour `list all` by omitting `--chat` only from the terminal, maintainer chat, or a technical-profile operator's home chat, the contexts the visibility rule in `docs/config-reference.md` leaves unscoped. Elsewhere, return the scoped list without mentioning other chats. Each list prints its bounded summary: pending rows and the last ten closed rows of the selected scope.

For cancellation from a channel turn, resolve the claim from that chat's scoped list and call `later cancel .claude-code-hermit <id> --chat <sourceKey>:<chat_id>`. From the terminal, omit `--chat`. Answer `unknown id` as not found without revealing contents. Closed rows remain terminal.

To send a result again, resolve the claim from the asking chat's scoped list, which retains its last ten closed rows. Reply with the saved state and reason into the row's `chat`, using channel-responder §0 to resolve the reply tool. Do not run the evidence command, change the verdict, add a row, or arm a trigger. For a pending or cancelled row, say that it is pending or cancelled instead. If the requested row is outside the window, say the result is no longer on hand.
