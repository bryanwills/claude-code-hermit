...
6. **Channel voice rule** (generalizes `claude-code-hermit:hermit-doctor`'s channel rule, `skills/hermit-doctor/SKILL.md` step 5, the paragraph naming what was blocked by kind): the message below is for the person who owns this hermit, not a developer. Never emit `PROP-NNN`/`S-NNN`, `operator_turns`, raw token counts, cron strings, or file paths. Speak in plain outcomes and counts.
