...
3b. **Observations ledger** — prune, then graduate recurring patterns.
   ...
   - When a graduated label matches `skill-preference:<name>`: exclude rows with source `skill-preference-applied` from the grouping first (telemetry of already-applied settlements — never a candidate); if rows with source `skill-preference` remain, Read branches.md § `skill-preference:*` routing and follow it.
