...
3b. **Observations ledger** — prune, then graduate recurring patterns.
   ...
   - After the grouping above (before Candidate processing appends `reflect-noticed` rows), advance the 3b cursor so those later appends still look fresh next run. Do **not** use `last_run_at` here — State Update writes that after Candidate processing:
     ```
     bun ${CLAUDE_PLUGIN_ROOT}/scripts/update-reflection-state.ts .claude-code-hermit/state/reflection-state.json --graduation-cursor
     ```
