...
- **No brief found (human/plugin or brief fully gone, moderate signal):** route by name class, testing the editable path **first**: `<name>` is the canonical bare name, and the two classes are not exclusive. An operator override at `.claude/skills/<name>/` does not shadow the plugin skill it overrides, both surface in the available-skills list (as `<name>` and `<plugin>:<name>`), so name class alone cannot tell them apart. The editable branch's precedence and the plugin branch's `No editable file` precondition are jointly what stop a re-recommendation of an override that already exists; neither is load-bearing alone:
  - **`.claude/skills/<name>/SKILL.md` exists (editable):** read the `## Lessons` section from each session listed in the graduated ledger rows' `session_id` fields to recover the corrected behaviors, as in the brief-found branch. Build a Tier 2 candidate with a `## Skill Improvement` section listing the component name and those corrected behaviors. Do not add an anchor line; state `moderate signal` as the confidence note. The candidate carries `Artifact: state/observations.jsonl`.
  - **No editable file, and `<name>` is an installed plugin skill (read-only)** — use the harness's available-skills list as the oracle, requiring a **namespaced** entry `<plugin>:<name>` (a bare `<name>` entry is an operator-space or bundled skill, never a plugin one, so it does not satisfy this branch): build a plain Tier 2 improvement candidate recommending an operator-space override skill in `.claude/skills/` or an upstream request, never an edit candidate for a file under the plugin cache. The candidate carries `Artifact: state/observations.jsonl`. When the available-skills list is not in context, the name class is unknown and this branch does not fire; the candidate follows the routing's existing plain-proposal outcome for a name matching neither class, asserting nothing about the skill being plugin-shipped and still never an edit candidate for a file under the plugin cache.
  - **Neither applies:** build a plain Tier 2 improvement proposal. The candidate carries `Artifact: state/observations.jsonl`.

  All three branches proceed via § Candidate processing.
...
- **No editable `.claude/skills/<name>/SKILL.md`, and `<name>` is an installed plugin skill (read-only):** plain Tier 2 improvement candidate recommending an operator-space override skill in `.claude/skills/` or an upstream request, never an edit candidate for a file under the plugin cache. Carries `Artifact: state/observations.jsonl`. The editable precondition is required, not decorative: same list-ambiguity between an override and the skill it overrides as § `skill-correction:*` routing above, so without it a settlement for an already-overridden skill lands here and re-recommends the override that already exists. When the available-skills list is not in context, the name class is unknown: this branch does not fire, and the settlement does not fall to **No skill covers the output** either — a plugin skill may well own it. Build a plain Tier 2 improvement candidate naming `<name>` and the settled content, asserting neither that the skill is plugin-shipped nor that nothing covers the output, and never an edit candidate for a file under the plugin cache.
