...
- **No brief found (human/plugin or brief fully gone, moderate signal):** route by name class using the harness's available-skills list as the installed-plugin-skill oracle:
  - **`<name>` is an installed plugin skill (read-only):** build a plain Tier 2 improvement candidate recommending an operator-space override skill in `.claude/skills/` or an upstream request, never an edit candidate for a file under the plugin cache. The candidate carries `Artifact: state/observations.jsonl`.
