...
- **No skill covers the output** (list readable, no file at that name, no entry for it): follow § Procedure capture's Preference ladder — extend an adjacent skill as a Tier 2 `## Skill Improvement` candidate, or the Tier-3 `## Skill Draft` path when nothing can absorb it.
