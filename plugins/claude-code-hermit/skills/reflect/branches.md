...
- **No brief found (human/plugin or brief fully gone, moderate signal):** route by name class, testing the editable path **first** — `<name>` is the canonical bare name, and an operator override at `.claude/skills/<name>/` shadowing a plugin skill matches both classes, so the plugin branch would otherwise swallow it and re-recommend an override that already exists:
