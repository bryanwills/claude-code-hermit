---
name: spawn-session
description: Spawn a background Claude Code helper in its own git worktree, watch it until idle, and relay its report to the operator. The launch path for a new helper session: it applies the configured Remote Control and the boot-written helper system prompt, which a hand-built `claude --bg` omits. Use when the operator says "spawn a helper", "spawn a new session", "run this in a background session", asks for a new session with `--model` or `--effort`, or names `/spawn-session`.
---
