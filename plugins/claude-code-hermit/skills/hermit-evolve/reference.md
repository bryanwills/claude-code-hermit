...
2. **Park the delta, always**: `diff -u <base_path> <the .bak> > .claude-code-hermit/state/docker-entrypoint.delta.<UTC-timestamp>.patch`. This is the operator's exact hunks even if every later step fails.
