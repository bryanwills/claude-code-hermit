...
5. **Sandbox note:** On Ubuntu 24.04+/26.04 hosts (and any host with `kernel.apparmor_restrict_unprivileged_userns=1`), the host kernel blocks unprivileged user-namespace creation inside the container via the AppArmor `docker-default` policy. This means `bwrap` cannot start in-container, so the Claude Code bash sandbox is effectively off regardless of settings. `hermit-start` detects this at boot and auto-disables the sandbox (writing `sandbox.enabled:false` to `.claude/settings.local.json`) so heartbeat and `/watch` monitors can spawn. Note: `sandbox.enableWeakerNestedSandbox` does NOT work around a blocked user-namespace — it only helps with the `/proc` access case. The container boundary (`cap_drop:ALL`, `no-new-privileges`, network policy) is the isolation layer here, per Anthropic recommendation (https://code.claude.com/docs/en/sandboxing). If this project's settings file has `sandbox.enabled:true`, the next container boot will auto-disable it.
