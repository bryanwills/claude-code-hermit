...
| `routine-cost` | Joins per-routine cost from `state/cost-index.json` (`by_source["routine:<id>"]`) with fire-turn counts from `state/routine-metrics.jsonl` (`started` + `skipped-*` events — skipped fires still accrue cost to the bucket) for enabled routines with ≥3 fires, computing `$/run`. | `warn` naming the routine when its `$/run` exceeds both 3× the peer median (the other routines' median) and `doctor.routine_cost_floor_usd` (default $2); `ok` when fewer than 2 routines have data or none exceed the threshold. |
