...
2.5. **Sandbox capability check** (twenty-fourth check, run after step 2):
   ...
   Add this as the twenty-fourth line to the summary.

3. Append a summary section to `.claude-code-hermit/sessions/SHELL.md` under a new
   `## Doctor Report (<ts>)` heading. Use the same twenty-four lines from steps 2 and 2.5. Place it
...
| `credential-expiry` | Executes each sibling plugin's `hermit-meta.json` `credentials[].expiry_probe` (bash, 5s timeout, one-line `OK`/`EXPIRES:<iso>`/`EXPIRED` protocol). Does not check the Claude Code session's own OAuth token — Claude Code refreshes that silently (~8h cycle, no operator action), so checking it produced false alarms. | `warn` if any plugin credential is EXPIRED or expires within 7 days, or if a probe times out or prints malformed output; `ok` otherwise (including when no plugin declares a credential). |
