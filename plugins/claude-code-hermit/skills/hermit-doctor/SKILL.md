...
| `voice-carrier` | Checks that the hermit's tone actually reaches the system prompt: `.claude/output-styles/hermit-voice.md` exists AND the winning `outputStyle` key names it. Resolution spans every persisted scope in Claude Code's precedence order — local, then project, then user (`CLAUDE_CONFIG_DIR`) — so a `/config` pick in `settings.local.json` that outranks a committed hermit key is caught. Reports only; seeding stops at project scope, since user scope cannot shadow a project write. | `ok` when both halves agree, when neither exists (an install that never adopted the voice), or when no voice file exists and a Claude Code built-in holds the key; `warn` when only one half is present, or when a voice file exists but another style is active — the operator's `/config` choice is never overwritten, just reported. |
