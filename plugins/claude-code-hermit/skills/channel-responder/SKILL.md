...
- **Harness command** (exactly `/compact`, `/clear`, `/model <arg>`, `/effort <arg>`, `/permission-mode <mode>`, `/advisor <model>`, `/doctor` (alias `/checkup`), or `/code-review […]` (alias `/review`))
  ...
  - `/doctor` and `/code-review` are relayed skill commands. They are covered by the silence rule above, so do not acknowledge them either; the hook runs them after this turn ends, and that later turn delivers the result to the requesting chat. `/doctor` requires settings authority. The hook refuses `ultra`, `--post`, and `--comment`, and its refusal reason is relayed instead of running the command. `--fix` is allowed — it only edits this hermit's own tree.
