...
- **Harness command** (exactly `/compact`, `/clear`, `/model <arg>`, `/effort <arg>`, `/permission-mode <mode>`, or `/advisor <model>`)
  ...
  - `/advisor <model>` pairs the main model with a second, typically stronger model that Claude Code consults at decision points (experimental, Anthropic API only). Valid arguments are `fable`, `opus`, `sonnet`, `off`, or a full model id — a rejected argument comes back as Claude Code's own inline text (e.g. "haiku cannot be used as an advisor. Valid options: fable, opus, sonnet, off"), so relay that verbatim rather than guessing why it failed. Unlike `/model`/`/effort` there is no cached-context pause to confirm. Unlike every other harness command here, the selection is **not** re-asserted at the next boot — Claude Code saves it to user settings, so it persists across restarts and each advisor call adds spend; `/advisor off` is the only way back.
  - A near-miss (`/model` with no argument, a bare `/advisor`, a bare `clear`, or prose mentioning one) is **not** intercepted — classify it under the categories below instead.
