...
### 4b. Verify the tz-shift helper

Run: `bun ${CLAUDE_PLUGIN_ROOT}/scripts/routines.ts tz-shift "0 4 * * *" "UTC"`

- Exit 0 and stdout is `0 4 * * *`: **PASS** `routines.ts tz-shift helper available`
- Exit non-zero or wrong stdout: **FAIL** `routines.ts tz-shift helper missing or broken — run /hermit-evolve`
