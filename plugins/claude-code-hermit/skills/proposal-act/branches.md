...
Rare-branch procedures for `proposal-act/SKILL.md`. Read only the section the dispatch names; each one is normative. Step numbers refer to `SKILL.md` § Accept Flow, and "the resolution algorithm" is `SKILL.md` § Resolving a Proposal ID.
...
1. Resolve the proposal file using `SKILL.md` § Resolving a Proposal ID, then read it.
