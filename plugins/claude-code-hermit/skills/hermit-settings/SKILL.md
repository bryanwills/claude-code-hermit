...
- **Maintainer chat only** — `boot-skill`, `remote`, `escalation`, `docker`, `artifact-backend`, and anything not named below. Changeable from the chat configured as `maintainer_channel_id` (allowlist-checked), or from a terminal. From any other chat: view-only.
- **Maintainer chat plus a confirmation code** — `permissions` (permission mode) and `env`. The gate issues a short code and denies the write; send that code to the maintainer chat and stop. It applies only after the operator echoes it back in a maintainer-chat message. Don't retry the setting in the same turn.
- **Terminal only, from every chat including the maintainer one** — `allowed_users`, `briefing_chat`, the DM chat, `maintainer_channel_id` itself, and any channel operation that replaces a channel wholesale (add/remove/primary). Show the current value, say plainly it changes only from a terminal session with `/claude-code-hermit:hermit-settings <argument>`, and change nothing.

Everything else in this skill is fair game over any channel — the write still goes through `settings-edit`, so it stays validated and audited. The authoritative list is the policy table in `scripts/channel-settings-gate.ts`, a `PreToolUse` hook that denies a write above the tier the turn's chat carries; treat a denial as this rule firing, relay its reason, and don't look for another route.
...
- **add <name>:** Prompt for `allowed_users` (paste user ID or skip) and `state_dir` (relative or absolute path — defaults to `.claude.local/channels/<name>`), then write the whole entry in one call: `set channels.<name> '{"enabled":true,"dm_channel_id":null,"default_chat_id":null,"allowed_users":[...],"state_dir":"..."}'`. Note: "Configure the channel token next: Docker → `/claude-code-hermit:docker-setup`; tmux or interactive → `/claude-code-hermit:channel-setup`."
