...
- **Maintainer chat plus a confirmation code** — `permissions` (permission mode), `env`, and `monitors` (each entry carries a shell command). The gate issues a short code and denies the write; send that code to the maintainer chat and stop. It applies only after the operator echoes it back in a maintainer-chat message. Don't retry the setting in the same turn.
