...
- **add <name>:** If the channel is already configured, point to `edit <name>` and write nothing. Otherwise prompt for `allowed_users` (paste user ID or skip) and `state_dir` (relative or absolute path, defaults to `.claude.local/channels/<name>`), then use `settings-edit set channels.<name> '{"enabled":true}'`, including `allowed_users` only when given and `state_dir` only when the operator supplied a non-default path. Do not include learned chat IDs or restate default paths. Channel enrollment raises Claude Code's native permission prompt. Note: "Configure the channel token next: Docker → `/claude-code-hermit:docker-setup`; tmux or interactive → `/claude-code-hermit:channel-setup`. Type it in a terminal or the Claude app."
...
- **edit <name>:** Sub-menu: "What to change? (allowed_users / briefing_chat / morning_brief / bind_home_chat / enabled / recall / operators / record / group / done)"
  ...
  - **bind_home_chat:** Set `channels.<name>.bind_home_chat` to `true` or `false`; absent means false. This applies to the next task in the home chat.
  ...
  - **group:** Collect every answer before writing: the Discord server channel or Telegram group ID (Discord: Developer Mode → right-click the chat → Copy Channel ID; Telegram: forward a message to `@userinfobot`, group IDs are negative integers). Discord threads inherit the parent policy, so use the parent channel ID. Ask whether a mention is required (default yes), who may trigger (default anyone, otherwise numeric user IDs), and whether history is shared with every chat (default no). Ask about passive recording only when mention is not required and no trigger IDs were given. When passive is Yes, `Read` that channel's `<state_dir>/access.json` (resolve `state_dir` from `channels.<name>.state_dir`, default `.claude.local/channels/<name>`, absolute against the project root); if `ackReaction` is a non-empty string, Ask: "Turn off the plugin-global seen-emoji? It also stops the emoji on your DMs for this channel. Turn off (recommended) / Keep."
    Missing or empty `ackReaction` asks nothing. Reject a chat ID equal to `maintainer_channel_id` before writing. Run `bun <plugin-root>/scripts/channel-access.ts <hermit-state-dir> group-add <name> <id> [--no-mention] [--allow …]` with the absolute Hermit state directory; pass `--no-mention` only when requested and `--allow id1,id2` only for explicit trigger IDs. This raises the native permission prompt. On any `ERROR|` token, report it and write nothing further; `needs-terminal` means pairing at a terminal. On `OK`, read the current `passive_chats` and `shared_chats` arrays (absent means `[]`), include this ID once for each Yes or remove it for No, and preserve every other ID. Preserve `passive_chats` when its question was not asked. Write the resulting arrays with `settings-edit set channels.<name>.passive_chats '["id",...]'` (only when asked) and `settings-edit set channels.<name>.shared_chats '["id",...]'`. Tell the operator the change applies on the next message in that chat, with no restart. On Turn off, also tell the operator to run `/<name>:access set ackReaction ""` at the terminal (`channel-access.ts` only supports `group-add`).
