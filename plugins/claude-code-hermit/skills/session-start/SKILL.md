...
6. Check if `.claude-code-hermit/sessions/NEXT-TASK.md` exists. If it does:
   - **Autonomous drain** (`config.always_on` is `true` AND `escalation` is `balanced` or `autonomous`): there is no operator to present to — auto-accept the prepared task. Use its `## Task` line as this session's task (as if the operator had accepted it) and adopt its `## Suggested Plan` in order as this session's plan. Step 1 gates the rest, so it runs before any edit. Then delete `NEXT-TASK.md`. This is the deterministic path the `session` Work-done flow (§6 step 7) and heartbeat Idle Agency invoke with a bare `session-start`.
