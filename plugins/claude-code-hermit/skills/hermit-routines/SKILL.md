...
- For an inline routine that sends a channel message, read only the Message formatting subsection of <pluginRoot>/skills/channel-responder/SKILL.md before invoking the skill, unless already in context. Apply it when preparing the send. Skip this read when no channel send is needed.
