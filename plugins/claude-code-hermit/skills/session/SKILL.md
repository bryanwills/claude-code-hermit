...
**Completion notification is the final step of this flow, not a substitute for it.** A session quiet for `heartbeat.stale_threshold` with no operator activity and no Progress Log entry is archived as `partial` at the next turn end or watchdog tick. Step 5 remains how Lessons and Artifacts get recorded.
