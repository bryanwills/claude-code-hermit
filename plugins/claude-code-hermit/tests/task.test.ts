...
it('standup groups stable identities and displays late queued shared and waiting work', async () => { const f = taskFixture(); try { const a = await f.open(['--due', '2020-01-01T00:00:00Z', '--requester-name', 'Person']); const b = await f.open(); await f.ok('block', [a.id, '--waiting-on', 'discord:u2', '--status-line', 'Waiting', '--next', 'Reply']); fs.writeFileSync(path.join(f.dir, '../.claude/cost-log.jsonl'), JSON.stringify({ task_id: a.id, task_ids: [a.id, b.id], bucket: 'tasks', cost_usd: 2, timestamp: new Date().toISOString() }) + '\n'); const result = await f.ok('standup'); const person = result.byPerson.find((p: any) => p.identity === 'discord:u1'); expect(person.name).toBe('Person'); expect(person.promised[0].id).toBe(a.id); expect(person.late[0].listing).toContain('shared'); expect(person.promised[1].listing).toContain('queued'); expect(result.byPerson.find((p: any) => p.identity === 'discord:u2').waiting[0].id).toBe(a.id); } finally { f.cleanup(); } });
