...
- `skills/diagnosing-bugs/` — diagnosis loop for hard bugs and performance regressions. Builds a red-capable feedback loop and runs it before hypothesising. Complements `/code-review` (static read) by running actual repros. Adapted from mattpocock/skills (MIT).
