...
- **Docs rewrite for v0.3.0** — full rewrites of `CLAUDE.md` (plugin-level), `README.md`, `docs/HOW-TO-USE.md`, `docs/WORKFLOW.md`; substantive trim of `docs/GIT-SAFETY.md` and `docs/RECOMMENDED-PLUGINS.md`; small structural edit to `CONTRIBUTING.md` (project-structure section). Deleted `docs/DEV-LOG-WATCH.md` (the dev-server log-tailing skill it documented is gone). All references to deleted features (dev-up/down/log-watch/status, dev-quality, dev-doctor, dev-cleanup, dev-adapt, dev-branch, implementer agent, watchdog system, `$HERMIT_AGENT_WORKTREE` topology, worktree token contract) are removed. New identity throughout: "language-agnostic safety layer + dev conventions for any agent your hermit uses to write code." `README.md` adds a v0.2.x → v0.3.0 migration section pointing operators at `feature-dev` and the native `Agent` tool as implementer replacements.

- **`/hatch` rewrite + remaining `scripts/lib/` deletion** — rewrote `skills/hatch/SKILL.md` (271 → ~155 lines) for v0.3.0: subsumed `/dev-adapt`'s wizard prompts (test_command, lint_command, format_command, protected_branches, pr_template_path), defaults to strict hook profile (with explicit opt-out, never downgrades an existing strict install), idempotent on re-run (every key offers `Keep current (<value>)` as the recommended option), reads `required_core_version` from `hermit-meta.json` instead of hardcoding the floor in skill prose, detects GitLab vs GitHub remote to seed `commands.pr_create` (preserves any existing operator override), filters companion-plugins options to only those not already installed. Dropped `routines`/`scheduled_checks` writes (no `/dev-cleanup` or `/dev-doctor` to schedule), `.worktreeinclude` write (no implementer agent to isolate), the heartbeat dev-checks block (referenced deleted skills), branch-naming detection (no consumer in the new wizard). Deleted `scripts/lib/resolve-command.js` (135 lines) + test (99 lines) and `scripts/lib/shell-utils.js` (7 lines) — `resolve-command` was a `command -v` wrapper used only by the now-deleted `/dev-doctor`. After this commit, `scripts/lib/` is empty.
