...
Routines `feed-brief-morning`, `feed-brief-evening`, and `weekly-digest` run on their cron schedules; they invoke domain skills directly, with schedules and `enabled` state in `config.json`. The `source-scout` routine runs unattended discovery and queues candidates for operator review.
