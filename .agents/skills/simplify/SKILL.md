...
3. **Respect house conventions.** If the session's `AGENTS.md` documents style rules or idiom preferences, they win over generic idioms — a reviewer suggesting f-strings is wrong if `AGENTS.md` says use `.format()`. AGENTS.md is the canonical place for project rules; subagents already see it in context, so no manual passthrough is needed.
...
- **Meaningfully different `new_string`** (different algorithm, different control flow, different intermediates) → check the **Principles** against each variant:
  ...
  - Does one variant respect *house conventions* (per AGENTS.md) while the other doesn't? → pick the conforming one.
