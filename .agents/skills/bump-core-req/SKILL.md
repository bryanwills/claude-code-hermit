...
description: >
  ...
  `required_core_version` and `requires["Codex-hermit"]` in `hermit-meta.json`, and
...
Per this monorepo's conventions (`AGENTS.md` → Conventions), the core version requirement lives in:
1. `plugins/<slug>/.Codex-plugin/hermit-meta.json` → `required_core_version` — **authoritative**, read by `doctor-check.ts` at runtime to detect incompatible siblings
2. `plugins/<slug>/.Codex-plugin/hermit-meta.json` → `requires["Codex-hermit"]` — documentation mirror
3. `plugins/<slug>/.Codex-plugin/plugin.json` → `dependencies[name=Codex-hermit].version` — native Codex resolver field
...
- `<fleet-slug>` — directory name of the fleet plugin under `plugins/` (e.g. `Codex-fitness-hermit`)
- `[version]` — optional target version like `1.0.26`. If omitted, read `plugins/Codex-hermit/.Codex-plugin/plugin.json` → `.version` and use that.
...
1. Glob `plugins/*/.Codex-plugin/plugin.json`. Collect directory names.
2. Remove `Codex-hermit` from the list — that's core, not a fleet plugin.
3. Ask via `AskUserQuestion`: "Which fleet plugin to update?" with one option per slug.

If `Codex-hermit` was explicitly passed as slug, abort: "Core doesn't depend on itself — pass a fleet plugin slug."

Validate `plugins/<slug>/.Codex-plugin/hermit-meta.json` exists. If not:
...
If no version arg, read `plugins/Codex-hermit/.Codex-plugin/plugin.json` → `.version`. This is the current shipped core version. Report it to the operator: "Autodetected core version: X.Y.Z".

### Step 2: Read current state

Read both files:
- `plugins/<slug>/.Codex-plugin/hermit-meta.json`
...
- `requires["Codex-hermit"]` (from hermit-meta.json) → same value, confirm they match
- `dependencies[name=Codex-hermit].version` (from plugin.json) → call this `old_dep_ver` (e.g. `^1.0.22`)
...
- Replace `"Codex-hermit": "<old_range>"` → `"Codex-hermit": ">=X.Y.Z"`
...
- Locate the exact string `"<old_dep_ver>"` that appears as the version value inside the `Codex-hermit` dependency entry. Because there is exactly one `Codex-hermit` dependency, replace the precise version string in context. Use enough surrounding text (the `"name": "Codex-hermit"` line or inline prefix) to make the replacement unambiguous if the version string is short.
