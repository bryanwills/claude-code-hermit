...
- **Token discipline (all plugins)**: hook stdout, skill-driven `Read`s, and helper-script output are injected into the operator's context — print verdict-sized digests, and never have a skill `Read` an unbounded surface (JSONL event logs, DBs) directly; front it with a script that returns a bounded summary. Full pattern and examples: `plugins/claude-code-hermit/CLAUDE.md` § Development constraints. (This file is itself the costliest token surface in the repo — loaded every session and re-seeded into every subagent — so keep additions here terse.)
